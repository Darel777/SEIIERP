    ...
    <div class="body">
      <el-tabs v-model="activeName" :stretch="true">
        <el-tab-pane label="待一级审批" name="PENDING_LEVEL_1">
          ...
          <div v-else>
            <salary_sheet_list :list="userList" :type="1" />
          </div>
        </el-tab-pane>
        <el-tab-pane label="待二级审批" name="PENDING_LEVEL_2">
          ...
          <div v-else>
            <salary_sheet_list :list="userList2" :type="2" />
          </div>
        </el-tab-pane>
        <el-tab-pane label="审批完成" name="PENDING_LEVEL_3">
          ...
          <div v-else>
            <salary_sheet_list :list="userList3" :type="3" />
          </div>
        </el-tab-pane>
        <el-tab-pane label="审批失败" name="PENDING_LEVEL_4">
          ...
          <div v-else>
            <salary_sheet_list :list="userList4" :type="4" />
          </div>
        </el-tab-pane>
      </el-tabs>
    </div>
...
import salary_sheet_list from "@/views/finance/salary_sheet_list";
import { get_salary_sheet_by_state, make_salary_sheet } from "@/network/purchase";
export default {
  components: {
    Layout,
    Title,
