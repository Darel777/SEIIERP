<template>
  <Layout>
    ...
    <el-button type="primary" size="medium" @click="dialogVisible = true">添加付款单</el-button>
    <div class="body">
      <el-tabs v-model="activeName" :stretch="true">
        <el-tab-pane label="待审批" name="PENDING_LEVEL_1">
          ...
          <div v-else>
            <purchase_sheet_list :list="userList" :type="1" />
          </div>
        </el-tab-pane>
        <el-tab-pane label="已审批" name="PENDING_LEVEL_2">
          ...
          <div v-else>
            <purchase_sheet_list :list="userList2" :type="2" />
          </div>
        </el-tab-pane>
        <el-tab-pane label="审批失败" name="PENDING_LEVEL_3">
          ...
          <div v-else>
            <purchase_sheet_list :list="userList3" :type="3" />
          </div>
        </el-tab-pane>
      </el-tabs>
    </div>
...
<!--          </el-form-item>-->
          ...
          <el-form-item
            v-for="(item, index) in new_collection_sheet_form.paymentSheetContent"
            :key="index"
            :label="'转账列表' + index">
            <div>
            ...
            <div style="margin-top: 10px">
              ...
              <el-button type="text" size="small" @click="func2()" v-if="index === new_collection_sheet_form.paymentSheetContent.length - 1">添加</el-button>
...
import purchase_sheet_list from "@/views/finance/purchase_sheet_list";
import {
  make_purchase_sheet,
  //制定付款单
...
export default {
  ...
  methods:{
    ...
    func1(){
      ...
      this.new_collection_sheet_form.paymentSheetContent.forEach((item) => {
      ...
      make_purchase_sheet(this.new_collection_sheet_form).then(_res => {
    ...
    func2(){
      this.new_collection_sheet_form.paymentSheetContent.push({});
    },
    func3(item){
      var index = this.new_collection_sheet_form.paymentSheetContent.indexOf(item)
      if(index!==-1){
        this.new_collection_sheet_form.paymentSheetContent.splice(index,1)
      }
    },
  }
};
