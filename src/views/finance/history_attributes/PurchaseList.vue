        ...
        <el-row >
          <el-col :span="18">
            ...
            <span style="margin-left: 10px">
              <el-tag v-if="type === 3&& (authorization() ===1 || authorization() === 2)" effect="dark" type='success'>审核通过</el-tag>
...
export default {
  ...
  methods: {
    getExcel(){
      ...
      let str='单号,供应商ID,操作员,备注,总额,审批状态\n'
      for(let i=0;i<this.res.length;i++){
        for(const key in this.res[i]){
          if(key.toString() !== 'purchaseSheetContent')
            str+=`${this.res[i][key] + '\t'},`;
        }
        str+='\n'
      }
